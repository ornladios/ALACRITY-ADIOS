...
ALError ALStorePOSIXOpenFiles(ALStore *store, const char *basename, _Bool is_reading) {
...
static void ALStorePOSIXExtendOffsetBuffers(ALStorePOSIXState *params, uint64_t new_capacity, _Bool copy_contents) {
    if (new_capacity == 0)
...
ALError ALStoreLoadGlobalMetadataPOSIX(ALStore *store) {
    if (store->access_mode != ACCESS_MODE_READ) {
        wprintf("[%s] Warning: attempt to load global metadata from file without being in read access mode.\n", __FUNCTION__);
    ...
    if (params->legacyFormat) {
        ...
        if (gmeta->num_partitions > params->partition_capacity)
            ALStorePOSIXExtendOffsetBuffers(params, gmeta->num_partitions, false);
...
ALError ALStoreWritePartitionPOSIX(ALStore *store, const ALPartitionData *part) {
    if (part->metadata.indexForm != ALInvertedIndex) {
        eprintf("[%s] Error: non-inverted index writing is not currently supported.\n", __FUNCTION__);
        return ALErrorSomething; // TODO: Error code
    }
    ...
    if (params->partition_capacity == gmeta->num_partitions) {
        ...
        ALStorePOSIXExtendOffsetBuffers(params, 0, true); // 0 -> double capacity, true -> maintain contents
    }
    ...
    //memstreamDestroy(&ms, true);
    fwrite(part->index, indexsize, 1, params->iindexfp); // Optimization, fix serialization later with file_stream_t
...
static uint64_t ALStorePOSIXGetGlobalHeaderSize(const ALStore *store) {
    ...
    if (params->legacyFormat) {
        ...
        eprintf("[%s] Error: new format unsupported at this time.\n", __FUNCTION__);
        return 0;
    }
}

#define METADATA_COPY_BUFSIZE 65536
static ALError ALStorePOSIXFinalizeMetadata(ALStore *store) {
    const ALGlobalMetadata *gmeta = &store->global_meta;
    ALStorePOSIXState *params = (ALStorePOSIXState*)store->impl_state;

    // First write the global header
    memstream_t ms = memstreamInitReturn(malloc(ALStorePOSIXGetGlobalHeaderSize(store)));
...
ALError ALStoreClosePOSIX(ALStore *store) {
    if (store->access_mode == ACCESS_MODE_WRITE) {
        int err = ALStorePOSIXFinalizeMetadata(store);
        if (err != ALErrorNone) return err;
    }
...
ALError ALStoreReadPartitionPOSIX(ALStore *store, ALPartitionData *part) {
    if (store->access_mode != ACCESS_MODE_READ) {
        wprintf("[%s] Warning: attempt to read partition data from file without being in read access mode.\n", __FUNCTION__);
        return ALErrorSomething; // TODO: Error code
    }
    ...
    if (params->legacyFormat) {
        ...
        fread(part->index, 1, indexSize, part->metadata.indexForm == ALInvertedIndex ? params->iindexfp : params->cindexfp);
...
ALError ALStoreOpenPartitionPOSIX(ALStore *store, ALPartitionStore *ps, _Bool open_excl) {
    ...
    pstore_state->data_offset = params->index_offsets[store->cur_partition];
    pstore_state->index_offset = params->data_offsets[store->cur_partition];
...
ALError ALPartitionStoreReadMetadataPOSIX(ALPartitionStore *ps, ALMetadata *meta) {
    ...
    if (params->legacyFormat) {
        ...
        eprintf("[%s] Error: new format not yet supported.\n", __FUNCTION__);
        return ALErrorSomething;
    }
}
...
ALError ALPartitionStoreReadDataBinsPOSIX(ALPartitionStore *ps, const ALMetadata *meta, bin_id_t low_bin, bin_id_t hi_bin, ALData *data) {
    ...
    const uint64_t first_bin_off = pstore_state->data_offset + meta->binLayout.binEndOffsets[low_bin] * insigbytes;
    const uint64_t last_bin_off = pstore_state->data_offset + meta->binLayout.binEndOffsets[hi_bin] * insigbytes;
...
ALError ALPartitionStoreReadIndexBinsPOSIX(ALPartitionStore *ps, const ALMetadata *meta, bin_id_t low_bin, bin_id_t hi_bin, ALIndex *index) {
    ...
    const char insigbytes = meta->elementSize - meta->significantBytes;
    _Bool is_inverted_index =  (meta->indexForm == ALInvertedIndex);
    const char index_elemsize = is_inverted_index ? sizeof(rid_t) : meta->significantBytes;
    const uint64_t first_bin_off = pstore_state->data_offset + meta->binLayout.binEndOffsets[low_bin] * index_elemsize;
    const uint64_t last_bin_off = pstore_state->data_offset + meta->binLayout.binEndOffsets[hi_bin] * index_elemsize;
    const uint64_t bin_read_len = last_bin_off - first_bin_off;
    FILE *fp = is_inverted_index ? params->iindexfp : params->cindexfp;
...
ALError ALPartitionStoreClosePOSIX(ALPartitionStore *ps) {
    ...
    FILE *indexfp = params->iindexfp; // TODO: Fix this somehow, either store the index form, or only support inverted index, or something
    ...
    fseek(indexfp, params->index_offsets[ps->source->cur_partition], SEEK_SET);
