...
LOG=/ccs/home/xzou2/alac_fastbit_comp/alac_multi_engine/script/fastbit.log
...
for ((i=0; i<${SEL}; i++ )) ; do
	...
	for ((k=1;k<11; k++ )); do
		...
		$FBIN $nval ${BASEDIR} ${WHERES4VAR[i]}   >> ${LOG} #--base-dir=$BASEDIR $FWHERE
	done
	echo >> ${LOG}
done
...
for ((i=0; i<${SEL}; i++ )) ; do
	...
	for ((k=1;k<11; k++ )); do
		BASEDIR="/lustre/widow2/scratch/xzou2/widow0-20130305/fastbit"${k}"/uvelindex/FastBit/P2/S3D/"
		$FBIN $nval ${BASEDIR} ${WHERES3VAR[i]}   >> ${LOG} #--base-dir=$BASEDIR $FWHERE
	done	
	echo >> ${LOG}
done
...
for ((i=0; i<${SEL}; i++ )) ; do
	...
	for ((k=1;k<11; k++ )); do
		BASEDIR="/lustre/widow2/scratch/xzou2/widow0-20130305/fastbit"${k}"/uvelindex/FastBit/P2/S3D/"
		$FBIN $nval ${BASEDIR} ${WHERES2VAR[i]}   >> ${LOG} #--base-dir=$BASEDIR $FWHERE
	done	
	echo >> ${LOG}
done
