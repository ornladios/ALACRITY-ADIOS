...
int main(int argc, char **argv) {
    if (argc < 3 || argc > 4) {
        fprintf(stderr, "Usage: %s <input filename> <output filename base> [compare to filename base]\n", argv[0]);
        return 1;
    }
    ...
    */
    ...
    //ALEncoderConfigure(&config, numDoubles, 2, 8, ALCompressedIndex, ALSerial);
    ...
    ALStore store;
    ALStoreOpenPOSIX(&store, outfilenamebase, "w", true);
    ...
    if (argc == 4) {
        const char *compfilenamebase = argv[3];
    ...
    ALStoreOpenPOSIX(&store, outfilenamebase, "r", true);
